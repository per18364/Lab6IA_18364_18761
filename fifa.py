...
def fifa():
    ...
    exp = Explorer(data, "Potential", keep_columns=['Age', 'Overall', 'Value', 'Wage', 'Special', 'Acceleration', "Dribbling", "Finishing", 'Agility', 'Ball control', 'Composure', 'Potential'], value_split=True)    
    data = exp.data
    # Convert to numerical values 
    exp.apply_func_column(numberFromMoney, "Value", "float")
    exp.apply_func_column(numberFromMoney, "Wage", "float")
    exp.apply_func_column(toNumber, "Acceleration", "float")#Acceleration -> Nummber or operation
    exp.apply_func_column(toNumber, "Agility", "float")#Agility -> Number or operation
    exp.apply_func_column(toNumber, "Ball control", "float")#Ball control -> Number or operation
    ...
    dtr = DTRegressor(exp)
    dtr.fit()
    y_real = list(Y_test[exp.target].values)
