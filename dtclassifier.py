    ...
    def build_tree(self, dataset, curr_depth=0):
        ...
        if num_samples >= self.min_samples_split and curr_depth <= self.max_depth:
            ...
            best_split = self.__get_best_split(
                num_samples, num_featues, dataset)
    ...
    def __get_best_split(self, num_samples, num_features, dataset):
        ...
        for feature_index in range(num_features):
            ...
            possible_threshold = self.__find_possibles_threshold(
                feature_values)  # Todas las posbilidades alm
            for threshold in possible_threshold:
                ...
                if len(dataset_left) > 9 and len(dataset_right) > 0:
                    ...
                    curr_info_gain = self.__information_gain(
...
if __name__ == "__main__":
    ...
    exp.reinstantiate_x_y()
    dtc = DTClassifier(exp, max_depth=5)
    dtc.fit()
    x_test = exp.X_test
    y_predict = dtc.predict(x_test)
    acc = exp.accuracy(y_predict)
    print(acc)
