...
class Node():
    def __init__(self, feature_index=None, threshold=None, left=None, right=None, var_red=None, info_gain=None, value=None) -> None:
...
class Explorer():
    ...
    def __init__(self, data: DataFrame, target, random_state=2, test_size=0.3, value_split=False, drop_columns=[], keep_columns=[]) -> None:
        ...
        self.clean_data(reinstantiateXY=False)
        # Separa el dataframe en X y y
        self.calculate_X_Y()  # Instantiate self.X and self.Y
        # Separa la data en datos de prueba
        # Instantiate self.X_train, self.X_test, self.X_val, self.Y_train, self.Y_test, self.Y_val
        self.calculate_split()

    def reinstantiate_x_y(self):
        self.calculate_X_Y()
        return self.calculate_split()
    ...
    def calculate_split(self):
        ...
        if self.value_split:
            self.X_test, self.X_val, self.Y_test, self.Y_val = train_test_split(
                self.X_test, self.Y_test, test_size=0.5, random_state=self.random_state)
        return self.X_train, self.X_test, self.X_val, self.Y_train, self.Y_test, self.Y_val

    def clean_data(self, reinstantiateXY=False):
    ...
    def correlation_in_dataset(self, relation_min=0.7):
        ...
        if not 0 < relation_min <= 1:
            relation_min = 0.7
        # Matriz de correlaciona a analizar
    ...
    def find_columns_with_regex(self, pattern):
        matches = self.data.applymap(
            lambda x: bool(re.search(pattern, str(x)))).any()
